...
- `s3_bucket`: `str` | The name of the S3 bucket where the file is stored.
- `expires_in`: `int = 120` | (Optional) The expiration time for the presigned URL in seconds. Default is 120.

### move_file_in_s3()

> `bool`

moves a file within S3 by copying to a new location and deleting the original:

<Code
    code={
`
from binaryrain_helper_cloud_aws.aws import move_file_in_s3

# Move a file within the same bucket

move_file_in_s3(
source_bucket="my-bucket",
source_filename="old-location/file.csv",
destination_filename="new-location/file.csv",
...
source_filename="data/export.json",
destination_filename="archive/export.json",
...
- `source_bucket`: `str` | The name of the S3 bucket where the source file is stored.
- `source_filename`: `str` | The name of the source file in S3.
- `destination_filename`: `str` | The name of the destination file in S3.
- `destination_bucket`: `str` | The name of the S3 bucket where the destination file will be stored.
