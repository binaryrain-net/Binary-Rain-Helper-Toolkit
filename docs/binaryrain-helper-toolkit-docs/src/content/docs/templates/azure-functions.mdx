...
<Code code={`PS C:\\> <venv>\\Scripts\\Activate.ps1`} lang="bash" />
...
If you want to secure your azure function so that only authorized people are allowed to access, then there are a few important settings.

First you have to follow the documentation instructions regarding [Authentication and authorization in Azure App Service and Azure Functions](https://learn.microsoft.com/en-us/azure/app-service/overview-authentication-authorization).

Once completed, the function app is only accessible for authorized users (depending on your configurations). But how to get the information about the signed in user? In the following example the goal is to extract the name of the user and the object identifier, which is often used for further graph api requests. An easy way to see all auth data and claims, go to your azure function and add `/.auth/me`. For further information about the principal information, see [here](https://learn.microsoft.com/en-us/azure/app-service/configure-authentication-user-identities).

<Code
  code={`principal_header = request.headers.get("x-ms-client-principal")
...
You can use an azure function also to send static html files as a response. This allows you to combine frontend and backend as a kind of fullstack endpoint. Click [here](https://github.com/binaryrain-net/Azure-Functions-Template/tree/main/Azure%20Functions/DemoFastApiReact), to visit the Azure Function Template repository example. It showcases an example on how to host react frontend and fastapi backend in an azure function.

An important note when using fastapi is to adjust the hosting settings. Since fastapi uses a different routing prefix config as azure function, you have to configure the host settings and add the following:
