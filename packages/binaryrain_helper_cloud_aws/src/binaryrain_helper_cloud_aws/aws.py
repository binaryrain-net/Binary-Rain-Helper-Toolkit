...
def get_secret_data(secret_name: str) -> dict:
    ...
    try:
        ...
        raise ValueError(f"Error getting secret for {secret_name} from SSM. Exception: {exc}") from exc
    except parameters.exceptions.TransformParameterError as exc:
        raise ValueError(f"Error transforming the secret for {secret_name}. Exception: {exc}") from exc
...
def get_app_config(AppConfig_environment: str, AppConfig_application: str, AppConfig_profile: str) -> dict:
    ...
    try:
        ...
        raise ValueError(f"Error loading configuration data from AppConfig. Exception: {exc}") from exc
    
    # load the configuration data
    try:
        app_config = json.loads(app_config)
    except json.JSONDecodeError as exc:
        raise ValueError(f"Error decoding configuration data from AppConfig. Exception: {exc}") from exc
...
def get_s3_presigned_url_readonly(filename: str, s3_bucket: str, expires_in: int = 120) -> str:
    ...
    try:
        ...
        raise ValueError(f"Could not create presigned URL. Check logs for more details. Exception: {exc}") from exc
